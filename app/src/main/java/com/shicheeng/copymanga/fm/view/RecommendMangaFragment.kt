...
class RecommendMangaFragment : Fragment() {

    private var _binding: MangaListBinding? = null
    private val binding get() = _binding!!
    private val listViewModel by viewModels<MangaListViewModel>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val pagingAdapter = MangaListAdapter()
        val footer = MangaLoadStateAdapter { pagingAdapter.retry() }
        val concatAdapter = pagingAdapter.withLoadStateFooter(footer)
        val gridLayoutManager =
            GridLayoutManager(requireContext(), 2, RecyclerView.VERTICAL, false).apply {
                applySpanCountWithFooter(concatAdapter)
            }
        binding.mangaListTotalRec.layoutManager = gridLayoutManager
        binding.mangaListTotalRec.adapter = concatAdapter
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                listViewModel.pageMangaRecommendFlow.collect {
